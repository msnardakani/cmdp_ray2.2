...
class AbstractSelfPacedTeacher:
    ...
    def get_task(self):
        return self.context_dist.get_weights()
    def set_task(self, task):
        self.context_dist.set_weights(task)
    ...
    def get_context(self):
        return {"mean": self.context_dist.mean(),
                "var": np.diag(self.context_dist.covariance_matrix()),
                'kl_div': self.target_context_kl()}
    ...
    def export_dist(self):
        ...
        return dict(target_mean=[self.context_dist.mean(),],
                    target_var=[self.context_dist.covariance_matrix(),],
...
class SelfPacedTeacherV2(AbstractTeacher, AbstractSelfPacedTeacher):
    ...
    def __init__(self, target_mean, target_variance, initial_mean, initial_variance, context_bounds, perf_lb,
                 max_kl=0.1, std_lower_bound=None, kl_threshold=None, use_avg_performance=False, update_interval = 5):
