...
from envs.gridworld_contextual import TaskSettableGridworld, coordinate_transform, box2discrete
...
if __name__ == "__main__":
    ...
    contexts = [(i, j) for i, j in itertools.product(range(ctx_lb[0], ctx_ub[0]+1), range(ctx_lb[1], ctx_ub[1]+1))]

    embeddings = np.array([[2 * (v[0] -ctx_lb[0])/ (ctx_ub[0]-ctx_lb[0]) - 1, 2 *( v[1]-ctx_lb[1]) / (ctx_ub[1]-ctx_lb[1]) - 1] for v in
                           contexts], dtype=np.float64)



    ctx_encoding_func = lambda ctx:np.array([2 * (ctx[('corridor', )][0]-ctx_lb[0])/ (ctx_ub[0]-ctx_lb[0])  - 1, 2 * (ctx[('corridor',)][1]-ctx_lb[1])/(ctx_ub[1]-ctx_lb[1]) - 1], dtype=np.float64)
    ctx_decoding_func = lambda z: {('corridor', ): contexts[np.argmin(np.sum((embeddings-z)**2, axis=1))]}
    ...
    config0 = {'size': size0,
                            'corridor': corridor0,
                            'region': {0: (1, 1), 1: (l-2, l-2)}}

    env_config_pars.append({'size': size0,
                            'corridor': corridor0,
                            'region': {0: (1, 1), 1: (l-2, l-2)}}  # 0 UL -> LR
                           )
    env_config_pars.append({'size': size0,
                            'corridor': corridor0,
                            'region': {0: (1, 1), 1: (1, l-2)}}  # 1 UL -> UR
                           )
    env_config_pars.append({'size': size0,
                            'corridor': corridor0,
                            'region': {0: (1, 1), 1: (l-2, 1)}}  # 1 UL -> LL
                           )
    ...
    for s, v in sets.items():
        for ctx_mode in ExpContextModes:
            start = ( env_config_pars[v[0]])['region'][0]
            target =( env_config_pars[v[0]])['region'][1]
            scaled_start= np.array([2 * (start[0]-1 )/ (size0[0]-2) - 1, 2 * (start[1]-2) / (size0[1]-2) - 1] , dtype=np.float64)
            perf_lb_fn = lambda iter, ctx_mean: -(l-min(contexts[np.argmin(np.sum((embeddings-ctx_mean)**2, axis=1))])+ max(contexts[np.argmin(np.sum((embeddings-ctx_mean)**2, axis=1))]))
            ctx_mean_transform = lambda  ctx_mean: embeddings[np.argmin(np.sum((embeddings-ctx_mean)**2, axis=1))]
            ...
            env_creator = lambda config: GMMCtxEnvWrapper(DiscreteCtxDictWrapper(TimeLimitRewardWrapper(
                ...
                key=[('corridor',)], ctx_decoder=ctx_decoding_func, ctx_encoder=ctx_encoding_func
            ...
            sp_config = {'kl_threshold': 8000,
                             'max_kl': tune.grid_search([ 0.05,]),
                             'perf_lb': -12,
                             'std_lb': tune.grid_search([.01,]),
                         }

            agent_config = [{'target_mean': ctx_encoding_func(flatten(env_config_pars[idx])),
                             'target_var': np.square([.001, ] * embedding_dim),
                             'init_mean': np.array([1.,-1.]),
                             'init_var': np.square([.5, ] * embedding_dim),
                             'prior': None,
            ...
            for i in range(num_agents):
                ...
                agent_config_sp[i]['min_episodes'] = 100
                ...
                agent_config_gsp[i]['ctx_mean_transform'] = None

                # agent_config_gsp[i]['std_lb'] = tune.grid_search([ 1,])
                agent_config_gsp[i]['min_episodes'] = tune.grid_search([64, ])
                agent_config_gsp[i]['update_interval'] = 1#tune.grid_search([   1])
                agent_config_gsp[i]['max_kl'] = tune.grid_search([ .1,])#tune.grid_search([   1])



            iteration_ts = step_per_iteration* num_agents
            ...
            config = (PPOConfig().environment(MAEnv, auto_wrap_old_gym_envs=False, env_config=env_config)
                      .training(train_batch_size=iteration_ts,  grad_clip=100, )#tune.grid_search([100, ]))
                      .framework('torch')
                      .resources(num_gpus=.12)
                      .rollouts(num_rollout_workers=1, num_envs_per_worker=32)
            ...
            if debug:
                ...
                for i in range(len(agent_config_gsp)):
                    agent_config_gsp[i]['min_episodes']=256
