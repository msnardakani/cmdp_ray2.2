...
if __name__ == "__main__":
    ...
    for s, v in sets.items():
        for ctx_mode in ExpContextModes:
            ...
            scaled_start= np.array([2 * (start[0]-1 )/ (size0[0]-1) - 1, 2 * (start[1]-1) / (size0[1]-1) - 1] , dtype=np.float64)
            perf_lb_fn = lambda iter, ctx_mean: -(max(np.abs(contexts[np.argmin(np.sum((embeddings-ctx_mean)**2, axis=1))]-np.array(start))) + corridor0[1])
            ...
            sp_config = {'kl_threshold': 8000,
                             'max_kl': tune.grid_search([ 0.01, 0.05]),
                             'perf_lb': -12,
            ...
            for i in range(num_agents):
                ...
                agent_config_gsp[i]['std_lb'] = tune.grid_search([  0.5, 1])
                agent_config_gsp[i]['curriculum'] = 'gaussian_self_pacedV4'

                agent_config_gsp[i]['perf_lb_fn'] = perf_lb_fn
                agent_config_gsp[i]['ctx_mean_transform'] = ctx_mean_transform
                ...
                agent_config_gsp[i]['max_kl'] = tune.grid_search([ .1, 0.25, 0.05])#tune.grid_search([   1])
