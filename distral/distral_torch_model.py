...
class DistralTorchModel(TorchModelV2, nn.Module):
    ...
    def __init__(
        ...
        super(DistralTorchModel, self).__init__(
    ...
    def get_distilled_model_outputs(
            self,
            model_out: TensorType,
            state_in: List[TensorType] = None,
            seq_lens: TensorType = None,
        ...
        if isinstance(model_out, dict) and "obs" in model_out:
            # Model outs may come as original Tuple observations
            if isinstance(self.distilled_model.obs_space, Box):
                model_out["obs"] = concat_obs_if_necessary(model_out["obs"])
            return self.distilled_model(model_out, state_in, seq_lens)
        else:
            if isinstance(self.distilled_model.obs_space, Box):
                model_out = concat_obs_if_necessary(model_out)
            return self.distilled_model({"obs": model_out}, state_in, seq_lens)
    ...
    def distilled_variables(self):
        return self.distilled_model.variables()
    ...
    def set_distilled_model(self, model):
        self.distilled_model = model




class DistilledTorchModel(TorchModelV2, nn.Module):
    """."""
    ...
    def __init__(
        ...
        nn.Module.__init__(self)
        self.distilled_model = model_config["distilled_model"]
    ...
    def forward(
        ...
        # model_out = self.model(self._last_flat_in)

        return self.distilled_model(input_dict["obs"]), state
...
class DistralCentralTorchModel(DistralTorchModel):
    ...
    def get_action_model_outputs(
        ...
        Returns:
            TensorType: Distribution inputs for sampling actions.
        """
        return self.get_distilled_model_outputs(model_out,state_in, seq_lens,)
