...
class BipedalWalker(gym.Env, EzPickle):
    ...
    def __init__(self, render_mode: Optional[str] = None, hardcore: bool = False):
        ...
        self.world = Box2D.b2World(gravity=(self.GRAVITY_X, self.GRAVITY_Y))
    ...
    def reset(
        ...
        *,
        ...
        self.scroll = 0.0
        self.lidar_render = 0 
        self.fd_polygon.friction= self.FRICTION
        self.fd_edge.friction= self.FRICTION
    ...
    def step(self, action: np.ndarray):
        ...
        if not self.observation_space.contains(state):
            state= np.clip(np.array(state, dtype=np.float32), a_min=self.observation_space.low, a_max= self.observation_space.high)
            reward = -100
            terminated = True
...
class BipedalWalkerCtx(BipedalWalker, TaskSettableEnv, EzPickle):
    metadata = {"render.modes": ["human", "rgb_array"], "video.frames_per_second": FPS}

    def __init__(
            self,
            render_mode: Optional[str] = "rgb_array",
            context=  np.array([80., 4., 6., 5.33, -10., 0., 2.5],dtype=np.float32),
    ):
        self.context=context
        self.MOTORS_TORQUE=context[0]
        self.SPEED_HIP=context[1]
        self.SPEED_KNEE=context[2]
        self.LIDAR_RANGE= context[3]
        self.GRAVITY_X=context[5]
        self.GRAVITY_Y=context[4]

        self.FRICTION=context[6]
    ...
    def set_task(self, task):
        self.context=task
        self.MOTORS_TORQUE=task[0]
        self.SPEED_HIP=task[1]
        self.SPEED_KNEE=task[2]
        self.LIDAR_RANGE= task[3]
        self.GRAVITY_X=task[5]
        self.GRAVITY_Y=task[4]

        self.FRICTION=task[6]
        return 
